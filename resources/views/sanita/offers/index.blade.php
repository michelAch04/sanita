...
@section('content')
<section id="offers" class="py-3 products-list {{ $isRtl ? 'rtl-container' : '' }}">
    <div class="px-2 py-2 gx-0 w-100">
        <h2 class="display-5 text-center mb-3 mb-md-5 section-title">{{ __('nav.offers') }}</h2>
        ...
        <div class="d-flex flex-md-row gap-md-1 flex-column">
            <div class="bg-secondary filter-container p-lg-3 p-2">
                <form method="GET" action="{{ url()->current() }}" id="sidebar-filterForm"
                    ...
                    <div class="filter-sidebar" id="filterSidebar">
                        ...
                        'products' => $offers, {{-- ✅ use offers here --}}
                        'eaPrices' => $eaPrices ?? collect(), {{-- ✅ optional price range --}}
                        'panel' => false])
                    </div>
                </form>
            </div>
            ...
            <div class="empty-filter-alert text-center py-5 px-4 my-5 rounded-4 shadow-sm">
                ...
                <div class="fs-4 fw-semibold mb-2 text-primary empty-filter-msg">
                    {{ __('No offers match your filters.') }}
                </div>
...
<link rel="stylesheet" href="{{ asset('css/products-list.css') }}?v=20260302-4" />
