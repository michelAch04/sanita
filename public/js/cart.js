document.addEventListener("DOMContentLoaded", function () {
                ...
                .then((data) => {
                    ...
                    if (data.success) {
                        ...
                        const totalPriceEl = document.querySelector(
                            `[data-id="${id}"] .total-price`
                        );
                        if (totalPriceEl) {
                            totalPriceEl.textContent = `$${Number(
                                data.item_total
                            ).toFixed(2)}`;
                        }
                        ...
                        showAjaxToast(
                            "warning",
                            window.cartMessages.updateFailed +
                                " " +
                                (data.message || "")
                        );
                    } else {
    ...
    document.querySelectorAll(".delete-item-form").forEach((form) => {
        const id = form.closest(".cart-item")?.dataset.id;
        if (!id) return;
        form.querySelector(".delete-item-btn").addEventListener(
            "click",
            (e) => {
                e.preventDefault();
                ...
                modalForm.onsubmit = function (event) {
                    ...
                    fetch(modalForm.action, {
                        ...
                        .then((data) => {
                            ...
                            if (data.success) {
                                ...
                                if (
                                    ...
                                    showAjaxToast(
                                        "success",
                                        window.cartMessages.removeSuccess
                                    );
                                    location.reload();
                                } else {
                                    showAjaxToast(
                                ...
                                showAjaxToast(
                                    "failed",
                                    window.cartMessages.removeFailed +
                                        (data.message || "")
                                );
                            }
                        })
                        .catch(() => {
                            ...
                            showAjaxToast(
                                "failed",
                                window.cartMessages.removeError
                            );
                        });
                };
            }
